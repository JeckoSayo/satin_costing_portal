...
def calculate_quote(data):
    ...
    if packaging_capacity <= 0 and packaging:
        packaging_capacity = D(packaging.packaging_capacity or 1)
    if packaging_capacity <= 0:
        packaging_capacity = Decimal("1")

    packaging_qty = Decimal("0")
    if packaging and quantity > 0:
        packaging_qty = (D(quantity) / packaging_capacity).to_integral_value(rounding=ROUND_CEILING)
    ...
    blade_wear_total = sheets_needed * D(settings.blade_wear_per_sheet) if use_cricut_cut else Decimal("0")
