...
class StickerSize(models.Model):
    ...
    def _paper_width(self):
        if not self.paper_size:
            return Decimal("8.27")
        return self.paper_size.cricut_safe_width_in if self.use_cricut_safe_area else self.paper_size.width_in

    def _paper_height(self):
        if not self.paper_size:
            return Decimal("11.69")
        return self.paper_size.cricut_safe_height_in if self.use_cricut_safe_area else self.paper_size.height_in

    @property
    def safe_fit(self):
        paper_w = self._paper_width()
        paper_h = self._paper_height()

        if self.width_in <= 0 or self.height_in <= 0:
            return 0

        normal = int((paper_w / self.width_in).to_integral_value(rounding=ROUND_FLOOR)) * int((paper_h / self.height_in).to_integral_value(rounding=ROUND_FLOOR))
        rotated = int((paper_w / self.height_in).to_integral_value(rounding=ROUND_FLOOR)) * int((paper_h / self.width_in).to_integral_value(rounding=ROUND_FLOOR))
    ...
    def best_for_costing(self):
        return self.safe_fit if self.use_cricut_safe_area else self.max_tight_fit
